...
async def extract_with_google_vision(file_path: str) -> dict:
    ...
    try:
        ...
        if not settings.ocr.google_application_credentials:
            raise HTTPException(
                status_code=400,
...
async def extract_with_aws_textract(file_path: str) -> dict:
    ...
    try:
        ...
        if not settings.ocr.aws_access_key_id or not settings.ocr.aws_secret_access_key:
            raise HTTPException(
                status_code=400,
...
async def decide_best_extraction_method(text: str, document_type: str) -> ExtractionMethod:
    ...
    if settings.llm.openai_api_key and len(text) > 500:
...
async def get_available_methods():
    ...
    return {
        "ocr_methods": {
            ...
            "google_vision": {
                "available": bool(settings.ocr.google_application_credentials),
            ...
            "aws_textract": {
                "available": bool(settings.ocr.aws_access_key_id and settings.ocr.aws_secret_access_key),
                "cost": "$1.50 por 1000 imágenes",
                "precision": "90-95%",
                "speed": "Medio",
